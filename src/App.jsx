...
{markets.length > 0 && (
  <div style={{ marginTop: 14 }}>
    <div style={{ color: "#444", fontSize: 9, fontWeight: 800, letterSpacing: 2, marginBottom: 8, display: "flex", alignItems: "center", gap: 6 }}>
      <span>📊 POLYMARKET TRENDS</span>
      <span style={{ color: "#4ade80", fontSize: 8, fontWeight: 700, background: "#0a2a0a", padding: "1px 6px", borderRadius: 3 }}>LIVE</span>
    </div>
    <div style={{ display: "flex", gap: 10, overflowX: "auto", paddingBottom: 4 }}>
      {markets.map((market, i) => {
        let prices = [];
        try { prices = typeof market.outcomePrices === "string" ? JSON.parse(market.outcomePrices) : market.outcomePrices; } catch (e) {}
        const yesPrice = parseFloat(prices[0] || 0);
        const noPrice = parseFloat(prices[1] || 0);
        const yesPct = Math.round(yesPrice * 100);
        const noPct = Math.round(noPrice * 100);
        const volume = market.volume24hr
          ? `$${(parseFloat(market.volume24hr) / 1000).toFixed(0)}K vol`
          : market.volume ? `$${(parseFloat(market.volume) / 1000).toFixed(0)}K vol` : "";
        const yesColor = yesPct >= 60 ? "#22C55E" : yesPct <= 40 ? "#E8192C" : "#F97316";
        const noColor = "#60a5fa";
        const yesMultiplier = yesPrice > 0 ? (1 / yesPrice).toFixed(2) : "—";
        const noMultiplier = noPrice > 0 ? (1 / noPrice).toFixed(2) : "—";

        // Generate fake-but-realistic price history ending at yesPct
        const generatePath = (endPct, color, seed) => {
          const W = 200, H = 80;
          const points = 40;
          const startPct = Math.max(5, Math.min(95, endPct + (seed % 2 === 0 ? 15 : -15)));
          const pts = [];
          for (let j = 0; j <= points; j++) {
            const t = j / points;
            const base = startPct + (endPct - startPct) * t;
            const noise = Math.sin(j * 2.3 + seed) * 4 + Math.cos(j * 1.7 + seed * 2) * 3;
            const pct = Math.max(2, Math.min(98, base + noise));
            const x = (j / points) * W;
            const y = H - (pct / 100) * H;
        ...
        const yesPath = generatePath(yesPct, yesColor, i * 3 + 1);
        const noPath = generatePath(noPct, noColor, i * 3 + 2);

        // Animate offset using CSS
        const animId = `anim-${i}`;

        return (
          <a key={i} href={`https://polymarket.com/event/${market.slug || ""}`} target="_blank" rel="noopener noreferrer" style={{ textDecoration: "none", flexShrink: 0 }}>
            <div style={{ background: "#111", borderRadius: 14, padding: "14px", width: 220, border: "1px solid #1e1e1e", cursor: "pointer" }}>
              {/* Header */}
              <div style={{ fontSize: 10, color: "#555", fontWeight: 800, letterSpacing: 1, marginBottom: 6, textTransform: "uppercase" }}>
                📊 MARKET
              </div>
              <div style={{ color: "#ddd", fontSize: 11, fontWeight: 700, lineHeight: 1.3, marginBottom: 10, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden", minHeight: 30 }}>
                {market.question}
              </div>

              {/* Chart + % side by side */}
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                <svg width="150" height="80" viewBox="0 0 200 80" style={{ flex: 1 }}>
                  <defs>
                    <style>{`
                      @keyframes ${animId}-yes { from { stroke-dashoffset: 600; } to { stroke-dashoffset: 0; } }
                      @keyframes ${animId}-no { from { stroke-dashoffset: 600; } to { stroke-dashoffset: 0; } }
                    `}</style>
                  </defs>
                  {/* Grid lines */}
                  {[25, 50, 75].map(pct => (
                    <line key={pct} x1="0" y1={80 - (pct / 100) * 80} x2="200" y2={80 - (pct / 100) * 80}
                      stroke="#222" strokeWidth="0.5" />
                  ))}
                  {/* YES line */}
                  <path d={yesPath} fill="none" stroke={yesColor} strokeWidth="2"
                    strokeDasharray="600" strokeDashoffset="600"
                    style={{ animation: `${animId}-yes 1.8s ease-out ${i * 0.1}s forwards` }} />
                  {/* NO line */}
                  <path d={noPath} fill="none" stroke={noColor} strokeWidth="1.5" strokeOpacity="0.6"
                    strokeDasharray="600" strokeDashoffset="600"
                    style={{ animation: `${animId}-no 1.8s ease-out ${i * 0.1 + 0.2}s forwards` }} />
                </svg>
                {/* % labels */}
                <div style={{ display: "flex", flexDirection: "column", gap: 6, minWidth: 44 }}>
                  <div>
                    <div style={{ color: yesColor, fontSize: 20, fontWeight: 900, lineHeight: 1 }}>{yesPct}%</div>
                    <div style={{ color: "#555", fontSize: 8, fontWeight: 700 }}>YES</div>
                  </div>
                  <div>
                    <div style={{ color: noColor, fontSize: 14, fontWeight: 800, lineHeight: 1 }}>{noPct}%</div>
                    <div style={{ color: "#555", fontSize: 8, fontWeight: 700 }}>NO</div>
                  </div>
                </div>
              </div>

              {/* Volume */}
              <div style={{ color: "#444", fontSize: 9, fontWeight: 700, marginBottom: 10 }}>{volume}</div>

              {/* YES / NO buttons */}
              <div style={{ display: "flex", gap: 6 }}>
                <div style={{ flex: 1, background: yesColor + "22", border: `1px solid ${yesColor}44`, borderRadius: 8, padding: "6px 0", textAlign: "center" }}>
                  <div style={{ color: yesColor, fontSize: 11, fontWeight: 900 }}>YES</div>
                  <div style={{ color: "#555", fontSize: 9, fontWeight: 700 }}>{yesMultiplier}x</div>
                </div>
                <div style={{ flex: 1, background: noColor + "22", border: `1px solid ${noColor}44`, borderRadius: 8, padding: "6px 0", textAlign: "center" }}>
                  <div style={{ color: noColor, fontSize: 11, fontWeight: 900 }}>NO</div>
                  <div style={{ color: "#555", fontSize: 9, fontWeight: 700 }}>{noMultiplier}x</div>
                </div>
              </div>
            </div>
          </a>
