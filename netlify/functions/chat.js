...
exports.handler = async (event) => {
  try {
    ...
    const rosterOverride = "\n\nTODAY IS: " + todayStr + "\n\nCRITICAL RULES:\n- Always answer roster, player, injury, and depth chart questions directly. Never mention the offseason, game schedules, or data availability.\n- For roster questions, do ONE web search then immediately write a concise answer under 300 words. Never explain that you are searching.\n- Use live roster data above when available. Otherwise search the web.\n- Never reference a player's old team. State their current team naturally.\n\nKNOWN CORRECTIONS:\n- Willson Contreras plays for the Boston Red Sox.\n- When listing a starting offense or depth chart, always format like this: first show a clean ordered list (QB, RB, FB if applicable, WR1, WR2, WR3, TE, LT, LG, C, RG, RT) with just the player name next to each position, then after the full list provide a brief news/notes section with relevant updates, injuries, or storylines for key players. Write TBD for any unknown positions.\n- When listing a starting offense or depth chart, format EXACTLY like this: show each position and player name on its own line (e.g. QB: Drake Maye), group WRs together as WR1, WR2, WR3: Name, Name, Name, write TBD for unknowns. After the list, add a single News & Notes section with 2-3 short punchy sentences about the most relevant storylines, transactions, or rumors. Keep the entire response under 200 words. No bullet points in the news section, just short sentences.";
